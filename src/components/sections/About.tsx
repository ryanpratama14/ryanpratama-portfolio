...
export default function About({ t, lang }: Props) {
	return (
		<article className="min-h-screen main-padding space-y-6" id="about">
			<GradientText
				text1={t.SECTIONS.aboutMe.split(" ")[0] ?? ""}
				text2={t.SECTIONS.aboutMe.split(" ")[1] ?? ""}
				bigger
			/>
				...
				<section className="flex relative w-fit divide-x justify-between gap-2">
					{getIdendityData(t).map((e, i) => {
						return (
							<p
								key={e}
								className={cn("font-monserrat label", { "pl-2": i !== 0 })}
							>
								{e}
							</p>
						);
			...
			<section className="md:w-[80%] lg:w-[70%] flex flex-wrap gap-3">
				{skillsData.map((e) => {
					return (
						<section
							key={e.icon}
							className="flex gap-2 text-sm xl:text-base items-center font-medium text-graydarker"
						>
								...
								<section className="relative md:h-[7rem] h-24 flex items-center w-full">
									...
									<section className="w-full flex flex-col px-4">
										...
										<small className="italic text-gray group-hover:text-white font-medium">
											...
											—{" "}
											{e.till?.toLocaleDateString(LANGUAGES[lang].locale, {
												month: "short",
												year: "numeric",
											}) ?? t.SECTIONS.present}
										</small>
