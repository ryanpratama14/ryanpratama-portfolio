  ...
  const card = (
    <section className="flex items-center text-left">
      ...
      <section className="pl-2 md:pl-2.5 flex flex-col gap-0.5">
        ...
        <small className="font-medium text-muted-foreground">{e.label}</small>
        <small className="text-muted-foreground/80">{formatWorkPeriod(e.location, e.type, e.since, e.till, s.SECTIONS.present)}</small>
      </section>
    </section>
  );
    ...
    return (
      <AccordionItem value={e.label}>
        ...
        <AccordionContent className="flex flex-col gap-0.5">
          ...
          <LinkButton unstyled href={e.href} className="w-fit font-medium border-b-1 border-primary text-blue-300">
