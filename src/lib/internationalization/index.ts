...
export const useLanguage = (lang: Lang) => {
  ...
  const currentTime = new Date().toLocaleTimeString(rest.locale, { hour: "2-digit", minute: "2-digit", second: "2-digit" });

  return { ...rest, s: t.static, isJapanese, isDefaultLang, baseUrl, currentTime, fn: useLanguageFn(lang) };
};
